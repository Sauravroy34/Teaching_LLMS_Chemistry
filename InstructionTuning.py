...
class MixedChemDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        # 1. IUPAC 50k
        iupac = load_dataset("hheiden/PubChem-124M-SMILES-SELFIES-InChI-IUPAC", split="train", streaming=True).take(20_000)
        # 2. Reactions 25k
        reactions = load_dataset("pingzhili/uspto-50k", split="train", streaming=True).take(20_000)
        # 3. Properties 15k
        properties = load_dataset("hheiden/PubChem-124M-SMILES-SELFIES-InChI-IUPAC", split="train", streaming=True).take(15_000)
        # 4. General chemistry knowledge 20k
        general = load_dataset("databricks/databricks-dolly-15k", split="train").take(15000)
        # 5. Raw SMILES replay 20k
        replay = load_dataset("antoinebcx/smiles-molecules-chembl", split="train", streaming=True).take(40_000)
        ...
        def collate_fn(batch):
            ...
            for item in batch:
                ...
                if task == 'iupac':
                    ...
                    prompt = PROMPT_GENERAL.format(instruction=item.get('instruction', ''))
                    target = item.get('response', '')
                    
                else:  # replay
                    text = item.get('smiles', '') or item.get('text', '')
                ...
                # Common path for prompted tasks
...
if __name__ == "__main__":
    ...
    print("Starting Mixed Instruction Tuning (130k samples - all tasks with short prompt style)...")
